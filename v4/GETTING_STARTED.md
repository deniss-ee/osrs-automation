...
If you want the full technical reference (every class, every gate, every gotcha discovered building the existing bots), that's `v4/ARCHITECTURE.md` — read this document first, then use that one as a lookup reference.
...
You don't need to understand every file in `v4/` to build a bot — you need to understand these five ideas:
...
- **An image anchor**: a small screenshot (`.png`, stored in `v4/Images/`) of something that appears in a fixed place, like a dialog box or the bank interface. `StaticAnchor.ImageAnchor(...)` searches for that image and waits until it appears.
...
Every coordinate/color/delay mentioned above lives in `v4/Config/auto-firemaking-v2.ini`, grouped under a comment block per phase.
...
2. **Copy its folder** under `v4/Bots/<YourBotName>/`, and copy its `.ini` under `v4/Config/`.
...
- `v4/ARCHITECTURE.md` — the full reference once you're past the basics.
- `v4/CURRENT_STATE.md` — a snapshot of what's built and verified right now.
