...
**2026-07-31: session pacing added (standard #30) — `Lib\Session.ahk`
(new file: `MaybeTakeBreak`, `NewSessionTimer`), wired into
`GatherBankLoop` via new `breakChance`/`breakMs`/`sessionLengthMs` opts
(all default off, existing callers unaffected).** Closes the one gap a
research pass (oldschoolscripts.com + an "Is AHK detectable" article)
found with zero current coverage — every other cited detection vector
(click timing, mouse path shape, click precision) was already handled
by standard #29. `Bots\woodcutting.ahk` got `TAKE_BREAKS`/
`BOUND_SESSION` config (both default `false`) but is NOT being flipped
on yet — sequenced after `micro\28-session-breaks.ahk` is live-confirmed,
per the `IdleWander` lesson, even though this change is lower-risk (pure
timing, no new mouse-movement code, testable with zero RuneLite
dependency via dummy gather/bank closures). Also reviewed a local
`v10\` folder (untracked, third-party oldschoolscripts.com toolkit
download, unrelated to this codebase) — nothing in it was portable or
worth adopting (its one mouse-movement algorithm is a WindMouse port,
the same algorithm this project already implemented and replaced).
NOT yet live-reconfirmed.
...
30. **Session pacing (`Lib\Session.ahk`, new file, added micro 28) —
    the one detection vector this codebase had zero coverage for.**
    Research pass across oldschoolscripts.com (a professional OSRS
    script seller) and an "Is AHK detectable in OSRS" article converged
    on the same conclusion: every commonly-cited detection vector this
    project could act on cheaply is already covered — fixed/uniform
    click timing (randomized reclickAfterMs, standard #29), straight-
    line mouse paths (HumanMove's minimum-jerk glide, standard #29),
    click-point precision (proportional BlockJitterPx, standard #29).
    The one gap: Jagex's detection ("Botwatch") is explicitly named as
    behavioral, not process-based, and the article calls out "prolonged
    repetitive action without breaks" and 20+ hours/day uptime as a
    named red flag; oldschoolscripts.com's advice is skill-dependent
    session caps (1-3h, shorter for high-ban-rate skills like Agility)
    and matching a normal human's play schedule. `GatherBankLoop`
    (`Lib\Steps.ahk`) ran forever until failure or F6 with zero
    exception — `Bots\woodcutting.ahk`'s `MAX_CYCLES := 0` confirmed
    this live.
    - Two independent, both-opt-in primitives in a NEW file
      (`Lib\Session.ahk`) rather than folded into `Core.ahk`/`Bot.ahk`/
      `Steps.ahk` — it's a distinct concern (pure timing/pacing policy)
      from Core's foundational primitives, Bot's hotkey harness, and
      Steps's explicitly-scoped "search + action" composites.
    - **`MaybeTakeBreak(opts)`** — per-call chance roll (`opts.chance`,
      0..1); on a hit, `Pause()`s a randomized "stepped away for a
      moment" duration (`opts.breakMs`, default `SESSION_BREAK_MS_DEFAULT`
      = 30s-3min) and returns true. Same "scalar or `[min,max]` array,
      rolled fresh at point of use" idiom as `TrackAndClick`'s
      `reclickAfterMs`/`NextReclickThreshold` (standard #29) — a
      distinct name/concept (probability gate, not a miss-recovery
      re-roll), same pattern reused deliberately.
    - **`NewSessionTimer(opts)`** — rolls a session length ONCE
      (`opts.sessionLengthMs`, default `SESSION_LENGTH_MS_DEFAULT` =
      1-2h, the article's suggested cap) and returns a closure the
      caller polls — same "maker" shape as `VerifySlotsAndDrop`
      (`Lib\Steps.ahk`).
    - **`GatherBankLoop` wiring**: new `breakChance`/`breakMs`/
      `sessionLengthMs` opts, ALL default off (`breakChance` 0;
      `sessionLengthMs` disabled via `HasOwnProp` absence, not a `0`
      sentinel, since a real value can be an array) — existing callers
      (`Bots\woodcutting.ahk`) are unaffected unless they opt in. Both
      checks land only at the gather→bank SEAM (after `bank()`
      succeeds, before the `maxCycles` check), never mid-unit — same
      non-interference principle the file's own header already states
      for "cut gathering short on a bank stall" decisions. Running out
      of session time returns `true` (clean stop), matching `maxCycles`'
      existing convention — it is explicitly NOT a failure.
    - **`Bots\woodcutting.ahk`**: `TAKE_BREAKS`/`BOUND_SESSION` config
      added, BOTH default `false` — same opt-in-flag convention as
      `CLICK_COOLDOWN_MS`. Deliberately NOT flipped on by default; only
      wired after micro 28 (below) is live-confirmed, per the
      `IdleWander` lesson (see standard #29's postmortem) of not
      trusting a new timing/behavior change in the real bot before
      isolated confirmation — though this feature is a fundamentally
      lower-risk class of change than `IdleWander` (pure scheduling, no
      new mouse-movement code, testable with dummy gather/bank closures
      and zero RuneLite dependency, unlike `IdleWander`'s
      only-judgeable-live cursor behavior).
    - **`micro\28-session-breaks.ahk`**: unlike most micros, needs NO
      real game state — same precedent as micro 26's do-nothing tick
      loop. Uses dummy gather/bank closures so only the new pacing
      surface is under test; deliberately aggressive test tuning
      (`breakChance=1.0`, seconds not hours) so a tester isn't waiting
      an hour to see a session end. Includes an explicit REGRESSION run
      (F7, breaks/session omitted) confirming old `GatherBankLoop`
      behavior is untouched — its handler resets `g_StopRequested` and
      wraps its own `try/catch BotStopped`, since `BindHotkeyHandler`
      (unlike `InstallBotHarness`'s own F5 path) does neither for
      `extraHotkeys`.
...
- `Lib\Steps.ahk` — `RightClickMenuItem`, `WaitThenClick` (shared
  ...
  (`breakChance`/`breakMs`/`sessionLengthMs` opts, standard #30, all
  default off) (ALL opts-object except `RunRestockPlan`, which is
  positional `(plan, ctrl)` — see standard #13).
...
- `Lib\Session.ahk` — `MaybeTakeBreak`, `NewSessionTimer`,
  `SESSION_BREAK_MS_DEFAULT`, `SESSION_LENGTH_MS_DEFAULT` (standard
  #30, micro 28).
...
- `micro\28-session-breaks.ahk` — `MaybeTakeBreak`/`NewSessionTimer`/
  `GatherBankLoop`'s `breakChance`/`breakMs`/`sessionLengthMs`
  validation (standard #30). No real game state needed — dummy
  gather/bank closures, same precedent as micro 26. NOT yet confirmed
  live.
